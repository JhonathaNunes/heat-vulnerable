...
@product_dp.route('/admin/product/file')
def download_file():
    file_id = request.args.get('file_id')
    # project_path = path.abspath(getcwd())
    file_path = path.join('../product_pic')

    print(file_path)

    try:
        return send_from_directory(file_path, file_id)
    except (exceptions.NotFound, FileNotFoundError):
        return send_file('static/images/no-photo.png')
